...
/**************************************
Date: December 25, 2023-sunlihang
Function: Radar result callback
功能: 回调超声数据
***************************************/
float radar_data = -1;

void turn_on_robot::radar_callback(const sensor_msgs::Range::ConstPtr &msg) {
    radar_data = msg->range*100;
    //std::cout << radar_data/100 <<"m"<< std::endl;
}

/**************************************
Date: Feb 1, 2024-sunlihang
Function: Pedestrain and red-green light result callback
功能: 回调行人和红绿灯数据
***************************************/
uint8_t pedestrain_data = 0x00;//int/u_int8
void turn_on_robot::pedestrain_callback(const std_msgs::Int8::ConstPtr &msg) {
    pedestrain_data = msg->data;
    printf("pedestrain data is %0x\n",pedestrain_data);
    //std::cout << "pedestrain_data is "<<pedestrain_data<< std::endl;
}

/**************************************
Date: December 26, 2023-sunlihang
Function: Chassis result callback
功能: 回调底盘数据
***************************************/
float chassis_data = 0;

void turn_on_robot::chassis_callback(const geometry_msgs::TwistStamped::ConstPtr &msg) {
    chassis_data = msg->twist.linear.x;
    //std::cout << radar_data/100 <<"m"<< std::endl;
}
...
{
  float temp = 0;
  temp = twist_aux.linear.x;
  //std::cout << temp << std::endl;
  bool stop_flag = 0;
  bool pedestrain_flag = 0;//定义两个用于超声停车和行人红绿灯的标志位
  ...
  // }
  //为超声停车的标志位进行检验赋值
  if(radar_data < 30 && radar_data >= 0)
  {
  	stop_flag = 1;
  }
  //为行人和红绿灯停车的标志位进行检验赋值
  bool pedestrain = (pedestrain_data & (1<<3)) >>3;//第五位为行人位,比如data为10，二进制为0b00001010,1左移三位为1000，然后做与运算得到00001000，右移为1
  bool light1 = (pedestrain_data & (1<<2)) >>2;//第六七两位共同表示红绿灯状态，00和01不用停车，10和11需要停车或减速
  bool light2 = (pedestrain_data & (1<<1)) >>1;
  std::cout <<"pedestrain 5th flag is:"<<pedestrain << std::endl;
  if(pedestrain == 1 || light1 == 1 )
  {
  	pedestrain_flag = 1;
  }
  std::cout <<"pedestrain final flag is:"<<pedestrain_flag << std::endl;
  if( stop_flag == 1 || pedestrain_flag == 1)//如果不符合启动条件，即接受到超声数据且距离障碍在阈值距离以上
  {
    //增加刹车逻辑，克服较大的惯性
    //chassis_data = chassis.linear.x;
    std::cout <<"chassis_data:"<<chassis_data << std::endl;
    if(chassis_data > 0)//如果车还有前进速度，则反向减速
    {
      transition = - (twist_aux.linear.x * 1000 * 3);
    }
    else//已经减速到0，则发布前进命令保持速度为0，发布后退命令时正常后退
    {
      transition=(twist_aux.linear.x>0) ? 0 : (twist_aux.linear.x*1000);//设置被障碍物阻挡时，可以后退但不能前进
    }
  }
  ...
  std::cout <<"send velocity transition:"<<transition << std::endl;
...
{ 
  ...
  */  
  ...
  //Record the position of the head and tail of the frame //记录帧头帧尾位置Cmd_Vel_Callback
  for(j=0;j<24;j++)
  {
    ...
    if(Receive_Data.Frame_Tail == FRAME_TAIL) //Verify the frame tail of the packet //验证数据包的帧尾
    {
      ...
      if(error == 0)
      {
        ...
        */
        ...
        //获取底盘速度同时将底盘速度作为话题发布出来-sunlihang
        chassis.header.stamp=ros::Time::now();
        chassis.twist.angular.z= Robot_Vel.Z;//-(Robot_Vel.Z);
        chassis.twist.linear.x = Robot_Vel.X;//-Robot_Vel.X;
        //ROS_INFO("%f",chassis.twist.angular.z);
        //ROS_INFO("%f",Robot_Vel.Z);
        //ROS_INFO_STREAM("1111116666DASDWQ");
        chassis_publisher.publish(chassis);
...
Function: Publish the Chassis data topic
功能: 发布底盘数据话题(于2023.12.24 排查底盘bug process has died过程中弃用)-sunlihang
***************************************/
void turn_on_robot::Publish_Chassis()
{
  //ROS_INFO_STREAM("SADSADAD474");
  short transition_16=0; //Intermediate variable //中间变量
  uint8_t i=0,check=0, error=1,Receive_Data_Pr[1]; //Temporary variable to save the data of the lower machine //临时变量，保存下位机数据
  static int count; //Static variable for counting //静态变量，用于计数
  Stm32_Serial.read(Receive_Data_Pr,sizeof(Receive_Data_Pr)); //Read the data sent by the lower computer through the serial port //通过串口读取下位机发送过来的数据
...
void turn_on_robot::Control()
{
  while(ros::ok())
  {
                                       ...
                                       //通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      //里程计误差修正Cmd_Vel_Callback
      Robot_Vel.X = Robot_Vel.X * odom_x_scale;
      Robot_Vel.Y = 0.0;

      if (odom_angular_z_is_steering)
      {
        float steering_angle = std::max(-max_steering_angle, std::min(max_steering_angle, Robot_Vel.Z));
        Robot_Vel.Z = Robot_Vel.X * tan(steering_angle) / wheel_base;
      }
      else
      {
        float z_scale = Robot_Vel.Z >= 0 ? odom_z_scale_positive : odom_z_scale_negative;
        Robot_Vel.Z = Robot_Vel.Z * z_scale;
      }

      //Speed * Time = displacement (odometer)
      //速度*时间=位移（里程计）
      const float delta_s = Robot_Vel.X * Sampling_Time;
      const float delta_theta = Robot_Vel.Z * Sampling_Time;
      Robot_Pos.X += delta_s * cos(Robot_Pos.Z + delta_theta * 0.5f); //Calculate the displacement in the X direction, unit: m //计算X方向的位移，单位：m
      Robot_Pos.Y += delta_s * sin(Robot_Pos.Z + delta_theta * 0.5f); //Calculate the displacement in the Y direction, unit: m //计算Y方向的位移，单位：m
      Robot_Pos.Z = NormalizeAngle(Robot_Pos.Z + delta_theta); //The angular displacement about the Z axis, in rad //绕Z轴的角位移，单位：rad 
      ...
      //Publish_Chassis();    //Pub the chassis topic //发布底盘话题
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("usart_port_name",  usart_port_name,  "/dev/serial/by-path/pci-0000:00:14.0-usb-0:1:1.0"); //Fixed serial port number //固定串口号
  ...
  private_nh.param<float>("wheel_base",    wheel_base,    0.313);
  private_nh.param<float>("max_steering_angle",    max_steering_angle,    0.523599);
  private_nh.param<bool>("odom_angular_z_is_steering",    odom_angular_z_is_steering,    true);
  ...
  //速度控制命令订阅回调函数设置
  Cmd_Vel_Sub     = n.subscribe("cmd_vel",1000, &turn_on_robot::Cmd_Vel_Callback, this); 
  radar_sub = n.subscribe("/ultrasonic_sensors/out/front_left", 100, &turn_on_robot::radar_callback,this);
  chassis_sub = n.subscribe("/Chassis",100,&turn_on_robot::chassis_callback,this);
  pedestrain_sub = n.subscribe("/simple_udp/simple_int8",100,&turn_on_robot::pedestrain_callback,this);
